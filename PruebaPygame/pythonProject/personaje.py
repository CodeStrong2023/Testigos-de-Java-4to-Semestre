...
while running:
    ...
    if keys[pygame.K_LEFT]: #Se presiona la tecla izq.
        player_pos.x -= player_speed
    if keys[pygame.K_RIGHT]: #Se presiona la tecla derecha
        player_pos.y += player_speed
    if keys[pygame.K_DOWN]: #Se presiona la tecla hacia abajo
        player_pos.y -= player_speed
    if keys[pygame.K_UP]: #Se presiona tecla hacia arriba
        player_pos.x += player_speed

    #Limpiar la pantalla
    screen.fill("green")

    #Dubijar el jugador que en este caso es un circulo rojo
